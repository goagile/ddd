...
class RedisIdeaRepository(IdeaRepository):
    ...
    def find_by_id(self, idea_id: str):
        ...
        author = self.client.hmget(idea_id, 'author')[0].decode()
...
class RateIdeaUseCase:
    ...
    def __init__(self, idea_repository: IdeaRepository, author_notifier: AuthorNotifier=None):
        self.idea_repository = idea_repository
        self.author_notifier = author_notifier
    ...
    def execute(self, request: RateIdeaRequest) -> RateIdeaResponse:
        ...
        if self.author_notifier:
            try:
                self.author_notifier.notify(idea.author)
            except Exception:
                raise NotificationNotSentException()
...
class NotificationNotSentException(Exception):
    pass
...
def rate_action():
    ...
    repository = RedisIdeaRepository(client=redis.Redis(host='localhost', port=6379, db=0))
